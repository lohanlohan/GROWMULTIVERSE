-- /bbscan — Brutal Bounce spawn zone scanner
-- Role 51 only
...
onPlayerCommandCallback(function(world, player, fullCommand)
    if world:getName():upper() ~= WORLD:upper() then return false end
    local args = {}
    for w in fullCommand:gmatch("%S+") do table.insert(args, w) end
    ...
    for i = 1, #zones, CHUNK do
        local line = {}
        for j = i, math.min(i + CHUNK - 1, #zones) do
            table.insert(line, zones[j])
        end
