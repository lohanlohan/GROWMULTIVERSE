...
local function isEffectiveDoctor(world, worldName, userID, player)
    return _G.isEffectiveDoctor(world, worldName, userID, player)
end
...
local function recordHospitalTreatment(worldName, maladyType, isSuccess)
    _G.recordHospitalTreatment(worldName, maladyType, isSuccess)
end
...
local function emitOperatingStatusBubble(world, x, y, row, now)
    ...
    local interval = tonumber(_G.OPERATING_STATUS_BUBBLE_INTERVAL_SEC) or 5
    ...
    world:onCreateChatBubble(x, y, text, 0)
end
...
local function findOperatingTileByPlayer(world, target)
    local operatingTableID = tonumber(_G.OPERATING_TABLE_ID) or 14662
...
function OperatingTable.processOperatingTablesInWorld(world, now)
    ...
    local spawnInterval = tonumber(_G.SURGBOT_SPAWN_INTERVAL_SEC) or (24 * 60 * 60)
    local operatingTableID = tonumber(_G.OPERATING_TABLE_ID) or 14662
...
_G.OperatingTable = OperatingTable
