-- MODULE
-- backpack.lua — Player backpack storage: store/retrieve items by category

local M      = {}
local DB     = _G.DB
local ItemCat = _G.ItemCategorizer

-- ============================================================================
-- STORAGE (per-player, cached in memory)
-- ============================================================================
local BP_CACHE = {}   -- { [uid] = {items=storedItems, dirty=bool} }

local function loadPlayer(uid)
    local data = DB.loadFeature("backpack")
    local raw  = data and data[tostring(uid)]
...
local function savePlayer(uid, storedItems)
    local data = DB.loadFeature("backpack")
    ...
    data[tostring(uid)] = items
    DB.saveFeature("backpack", data)
end

local function getState(player)
    local uid = player:getUserID()
    if not BP_CACHE[uid] then
        BP_CACHE[uid] = { storedItems = loadPlayer(uid) }
    end
    return BP_CACHE[uid]
end
...
local function addToBackpack(player, itemID, count)
    count = math.min(tonumber(count) or 1, player:getItemAmount(itemID) or 0)
    if count <= 0 then return 0 end
    player:changeItem(itemID, -count, 0)
    local state = getState(player)
    state.storedItems[itemID] = (state.storedItems[itemID] or 0) + count
    savePlayer(player:getUserID(), state.storedItems)
    return count
end
...
local function sendMainPanel(player)
    ...
    for _, cat in ipairs(ItemCat.LIST) do
        ...
        d = d .. "add_button_with_icon|bpCat_" .. cat .. "|" .. display .. "|is_count_label|" .. icon .. "|" .. cnt .. "|left|\n"
        idx = idx + 1
        if idx % ITEMS_PER_ROW == 0 then d = d .. "add_custom_break|\n" end
    end
...
-- ============================================================================
-- PANEL: STORE (item picker)
-- ============================================================================
local function sendStorePanel(player)
    player:onDialogRequest(
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wStore Items``|left|4516|\n" ..
        "add_spacer|small|\n" ..
        "add_item_picker|selectedItem|Select Item|Choose an item to store|\n" ..
        "add_quick_exit|\n" ..
        "end_dialog|bp_store|||"
    )
end
...
local function sendCategoryPanel(player, catName, page)
    ...
    table.sort(items, function(a, b) return nameCache[a.id] < nameCache[b.id] end)
    ...
    d = d .. "add_spacer|small|\n"
    if page > 1        then d = d .. "add_button|btn_cat_prev|< Prev|noflags|0|0|\n" end
    if page < totalPages then d = d .. "add_button|btn_cat_next|Next >|noflags|0|0|\n" end
    if totalItems > 0  then d = d .. "add_button|btn_clear_cat|`4Clear All``|noflags|0|0|\n" end
...
local function sendClearConfirmPanel(player, catName)
    ...
    local d = "set_default_color|`o\n"
    d = d .. "add_label_with_icon|big|`4Clear All " .. catName .. "``|left|" .. icon .. "|\n"
    d = d .. "add_spacer|small|\n"
...
local function sendItemActionPanel(player, itemID)
    ...
    d = d .. "add_spacer|small|\n"
    if maxInv > 0 then d = d .. "add_button|btn_takeall|`2Take All|noflags|0|0|\n" end
...
-- ADMIN HELPER
-- ============================================================================
local function getOnlineByUID(uid)
...
local function sendAdminCategoryPanel(admin, catName, page)
    local aState = getState(admin)
    local target = aState.editTarget and getOnlineByUID(aState.editTarget) or nil
    if not target then
...
local function sendAdminItemActionPanel(admin, itemID)
    ...
    d = d .. "add_spacer|small|\n"
    if maxInv > 0 then d = d .. "add_button|btn_admintakeall|`2Take All to Inventory|noflags|0|0|\n" end
...
onPlayerCommandCallback(function(world, player, full)
    local args = {}
    for w in full:gmatch("%S+") do table.insert(args, w) end
...
onPlayerCommandCallback(function(world, player, full)
    ...
    savePlayer(target:getUserID(), targetState.storedItems)

    local cat = ItemCat.getCategory(itemID)
    player:onConsoleMessage("`2Gave `w" .. amount .. "x " .. item:getName() .. " `2to `w" .. target:getName() .. "`2's backpack! `o(Category: " .. cat .. ")")
    target:onConsoleMessage("`2You received `w" .. amount .. "x " .. item:getName() .. " `2in your backpack from `w" .. player:getName() .. "`2! `o(Category: " .. cat .. ")")
    return true
end)

-- ============================================================================
-- DIALOG CALLBACK: PLAYER PANELS
-- ============================================================================
onPlayerDialogCallback(function(world, player, data)
    ...
    if dn == "bp_main" then
        if btn == "btn_store_items" then sendStorePanel(player); return true end
        local cat = btn:match("^bpCat_(.+)$")
        if cat then sendCategoryPanel(player, cat); return true end
        return true
    end
    ...
    if dn == "bp_cat" then
        if btn == "btn_back_main"  then sendMainPanel(player); return true end
        if btn == "btn_cat_prev"   then sendCategoryPanel(player, state.currentCat or "Block", (state.currentPage or 1) - 1); return true end
        if btn == "btn_cat_next"   then sendCategoryPanel(player, state.currentCat or "Block", (state.currentPage or 1) + 1); return true end
        if btn == "btn_clear_cat"  then sendClearConfirmPanel(player, state.currentCat or "Block"); return true end
        local itemID = tonumber(btn:match("^bpItem_(%d+)$"))
        if itemID then sendItemActionPanel(player, itemID); return true end
        return true
    end
    ...
    if dn == "bp_item" then
        ...
        if not itemID then sendMainPanel(player); return true end
        ...
        local rawAmt = math.max(1, tonumber(data["bp_amount"]) or 1)
...
onPlayerDialogCallback(function(world, player, data)
    ...
    if dn == "bpadmin_main" then
        ...
        if cat then sendAdminCategoryPanel(player, cat, 1) end
        return true
    end
    ...
    if dn == "bpadmin_cat" then
        ...
        if btn == "btn_admincat_prev"  then sendAdminCategoryPanel(player, aState.adminCurrentCat or "Block", (aState.adminCurrentPage or 1) - 1); return true end
        if btn == "btn_admincat_next"  then sendAdminCategoryPanel(player, aState.adminCurrentCat or "Block", (aState.adminCurrentPage or 1) + 1); return true end
        if btn == "btn_admincat_clear" then sendAdminClearConfirmPanel(player, aState.adminCurrentCat or "Block"); return true end
        local itemID = tonumber(btn:match("^bpadminItem_(%d+)$"))
        if itemID then sendAdminItemActionPanel(player, itemID); return true end
        return true
    end
    ...
    if dn == "bpadmin_clear_confirm" then
        ...
        local target  = aState.editTarget and getOnlineByUID(aState.editTarget) or nil
        if not target then player:onConsoleMessage("`4Target player went offline."); return true end
        ...
        if btn == "btn_adminconfirm_clear" then
            ...
            savePlayer(target:getUserID(), tState.storedItems)
    ...
    if dn == "bpadmin_item" then
        ...
        if not itemID then sendAdminMainPanel(player, target); return true end
        local tState = getState(target)
        ...
        if btn == "btn_admintakeall_bp" then
            ...
            if amount > 0 then
                ...
                if taken > 0 then
                    ...
                    savePlayer(player:getUserID(), myState.storedItems)
                    local name = getItem(itemID) and getItem(itemID):getName() or "item"
...
onPlayerDisconnectCallback(function(player)
    BP_CACHE[player:getUserID()] = nil
end)

-- ============================================================================
-- GLOBAL HOOKS (cross-feature)
-- ============================================================================
