...
local RENT_ENTRANCE_ID        = 25020  -- tile asli, collision none (renter/owner bisa lewat)
local RENT_ENTRANCE_CLOSED_ID = 25026  -- visual solid untuk non-renter (collision normal)
local ROLE_DEVELOPER       = 51
local WORLD_LOCK_ID        = 242
local DIAMOND_LOCK_ID      = 1796
local BGL_ID               = 7188
local GGL_ID               = 0      -- pending (isi nanti)
...
local RENT_JSON = "rent_entrance_data.json"
local DB_CACHE  = nil

local function readDB()
    if DB_CACHE then return DB_CACHE end
    if not file.exists(RENT_JSON) then DB_CACHE = {}; return DB_CACHE end
    DB_CACHE = json.decode(file.read(RENT_JSON)) or {}
    return DB_CACHE
end

local function writeDB(data)
    DB_CACHE = data
    file.write(RENT_JSON, json.encode(data))
end
...
local function getName(player)
    if type(player) ~= "userdata" then return "Unknown" end
    if type(player.getName) == "function" then return tostring(player:getName()) end
    return "Unknown"
end
...
local function isWorldOwner(world, player)
    ...
    return world:getOwner(player) == true
end

local function isDeveloper(player)
    return type(player) == "userdata" and player:hasRole(ROLE_DEVELOPER) == true
end

local function isPrivileged(world, player)
    return isWorldOwner(world, player) or isDeveloper(player)
end

local function getPlayerItemCount(player, itemID)
    if type(player) ~= "userdata" or itemID <= 0 then return 0 end
    if player.getItemAmount then return tonumber(player:getItemAmount(itemID)) or 0 end
    return 0
end
...
-- =======================================================
-- HELPERS
-- tile:getPosX/Y() return pixel coords, world:getTile() butuh tile grid (pixel/32)
-- =======================================================
...
-- Particle di posisi tile (x, y dalam pixel)
...
local function spawnParticleAll(world, x, y)
    local players = world:getPlayers()
    if type(players) ~= "table" then return end
    for _, p in pairs(players) do spawnParticle(p, x, y) end
end

-- Kirim visual 25020 (passable) ke player yang punya akses
...
-- Kirim visual 25026 (solid) ke player yang tidak punya akses
...
-- Reset visual solid (25026) ke semua player (saat tidak ada renter aktif)
...
-- Refresh visual setiap player masuk world
...
showOwnerPanel = function(world, player, worldName, x, y)
    ...
    player:onDialogRequest(d, 0, function(cbWorld, cbPlayer, data)
        ...
        -- Simpan harga + max setiap submit
...
showRenterListPanel = function(world, player, worldName, x, y)
    ...
    if not hasAny then
        d = d .. "add_smalltext|`7No active renters.|\n"
    end
        ...
        if btn and btn:match("^btn_rmv_") then
            ...
            if uid > 0 then
                ...
                if e and e.renters[key] then
                    ...
                    e.renters[key] = nil
                    saveEntrance(worldName, x, y, e)
                    -- Revoke tile access kalau online
    ...
    if type(wPlayers) == "table" then
        for _, p in pairs(wPlayers) do
            if not isPrivileged(world, p) then
                ...
                d = d .. "add_button|btn_pickp_" .. pid .. "|`w" .. getName(p) .. tag .. "|noflags|0|0|\n"
            end
        end
    end
    if not anyAdded then
        d = d .. "add_smalltext|`7No eligible players in world.|\n"
    end
    ...
    player:onDialogRequest(d, 0, function(cbWorld, cbPlayer, data)
        ...
        if btn and btn:match("^btn_pickp_%d+") then
            ...
            if targetUID > 0 then
                ...
                if type(wps) == "table" then
                    for _, p in pairs(wps) do
                        if getUserID(p) == targetUID then
                            targetName = getName(p); break
                        end
                    end
                end
    ...
    player:onDialogRequest(d, 0, function(cbWorld, cbPlayer, data)
        ...
        if btn and btn:match("^btn_picko_") then
            ...
            if opt then
                ...
                -- Grant tile access kalau target online
                local wps = cbWorld:getPlayers()
                if type(wps) == "table" then
                    for _, p in pairs(wps) do
                        if getUserID(p) == targetUID then
...
showPlayerConfirmPanel = function(world, player, worldName, x, y, opt, price)
    ...
    player:onDialogRequest(d, 0, function(cbWorld, cbPlayer, data)
        ...
        if btn == "btn_cfm_yes" then
            ...
            if curPrice == 0 then
                safeBubble(cbPlayer, "`4This option is no longer available."); return true
            end
            if not isExisting and active >= maxR then
                safeBubble(cbPlayer, "`4This entrance is full!"); return true
            end
            if getTotalWLEquiv(cbPlayer) < curPrice then
                safeBubble(cbPlayer, "`4Not enough World Locks!"); return true
            end
            if not deductWL(cbPlayer, curPrice) then
                safeBubble(cbPlayer, "`4Payment failed."); return true
            end
            ...
            e.renters[renterKey] = {
                name       = getName(cbPlayer),
                option     = opt.key,
                expires_at = expiresAt,
            }
            saveEntrance(worldName, x, y, e)

            -- Grant tile access langsung
...
-- =======================================================
-- PLAYER STATUS PANEL (sudah punya akses aktif)
-- =======================================================
...
showPlayerStatusPanel = function(world, player, worldName, x, y)
    ...
    if renter and isRenterValid(renter) then
        ...
        -- Akses expired → revoke dan tawari beli lagi
        revokeVisualAccess(world, player, x, y)
...
-- Wrench:
--   Privileged  → owner config panel
--   Renter aktif → status panel
--   Lainnya      → buy panel
...
-- Punch:
--   Privileged + kondisi OK  → izinkan break
--   Privileged + ada renter/WL → prevent break + bubble
--   Non-privileged           → prevent break + bubble
onTilePunchCallback(function(world, player, tile)
    ...
    if isPrivileged(world, player) then
        ...
        -- Kondisi bersih → izinkan break normal
        return false
    end

    -- Non-privileged → prevent break entrance
    player:adjustBlockHitCount(10)
...
-- Placement:
--   Hanya owner, hanya di world locked
onTilePlaceCallback(function(world, player, tile, placingID)
    ...
    -- Delay singkat agar tile selesai ter-place dulu, baru kirim visual solid ke non-renter
...
-- Break cleanup: revoke semua tile access + hapus data
onTileBreakCallback(function(world, player, tile)
    ...
    if type(wps) == "table" then
        for _, p in pairs(wps) do
            revokeVisualAccess(world, p, x, y)
        end
    end
    deleteEntrance(getWorldName(world, player), x, y)
end)

-- Enter world → grant tile access untuk privileged dan renter valid
...
-- World tick → cek expired renters, revoke tile access
onWorldTick(function(world)
    local worldName = world:getName()
    local db        = readDB()   -- in-memory cache, no file I/O
    local players   = world:getPlayers()
    if type(players) ~= "table" then return end
    ...
    for key, ent in pairs(db) do
        ...
        if wn == worldName and type(ent.renters) == "table" then
            ...
            if changed then
                ...
                if countActiveRenters(ent) == 0 then
                    resetVisualForAll(world, tx, ty)
                end
            end
        end
    end
